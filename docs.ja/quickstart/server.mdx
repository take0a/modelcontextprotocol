...
このチュートリアルでは、シンプルなMCP天気予報サーバーを構築し、ホストであるClaude for Desktopに接続します。
まずは基本的な設定から始め、その後、より複雑なユースケースへと進んでいきます。

### 構築するもの

多くのLLMは現在、天気予報や悪天候警報を取得する機能を備えていません。MCPを使ってこの問題を解決しましょう！

`get_alerts`と`get_forecast`という2つのツールを公開するサーバーを構築します。そして、そのサーバーをMCPホスト（今回はClaude for Desktop）に接続します。
...
サーバーはどのクライアントにも接続できます。ここではシンプルさを重視してClaude for Desktopを選択しましたが、[独自のクライアントを構築する](/quickstart/client)ガイドや[その他のクライアントの一覧はこちら](/clients)もご用意しています。
...
1. **リソース**: クライアントが読み取り可能なファイル形式のデータ（API レスポンスやファイルコンテンツなど）
2. **ツール**: LLM から（ユーザーの承認を得て）呼び出すことができる関数
3. **プロンプト**: ユーザーが特定のタスクを実行するのに役立つ、事前に作成されたテンプレート
...
天気予報サーバーの構築を始めましょう！[構築するサーバーの完全なコードは、こちらでご覧いただけます。](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-python)
...
MCP サーバーを実装する際は、ログ記録の処理方法に注意してください。

**STDIO ベースのサーバーの場合:** 標準出力 (stdout) への書き込みは絶対に行わないでください。これには以下のものが含まれます。
...
stdout への書き込みは JSON-RPC メッセージを破損させ、サーバーを動作不能にします。

**HTTP ベースのサーバーの場合:** 標準出力へのログ記録は HTTP レスポンスに影響を与えないため、問題ありません。

### ベストプラクティス

1. 標準エラー出力またはファイルに書き込むログライブラリを使用します。
...
`uv` コマンドが確実に実行されるように、ターミナルを再起動してください。
...
次に、National Weather Service API から取得したデータのクエリとフォーマットを行うためのヘルパー関数を追加しましょう。
...
サーバーが完成しました！ `uv run weather.py` を実行して MCP サーバーを起動すると、MCP ホストからのメッセージをリッスンします。
...
Claude for Desktop はまだ Linux ではご利用いただけません。
Linux ユーザーは、[クライアントの構築](/quickstart/client) チュートリアルに進み、構築したサーバーに接続する MCP クライアントを構築できます。

</Note>

まず、Claude for Desktop がインストールされていることを確認してください。[最新バージョンはこちらからインストールできます。](https://claude.ai/download) 既に Claude for Desktop をお持ちの場合は、**最新バージョンにアップデートされていることを確認してください。**

使用する MCP サーバーに合わせて Claude for Desktop を設定する必要があります。これを行うには、テキストエディターで `~/Library/Application Support/Claude/claude_desktop_config.json` にある Claude for Desktop アプリの設定ファイルを開きます。ファイルが存在しない場合は作成してください。
...
次に、`mcpServers` キーにサーバーを追加します。MCP UI 要素は、少なくとも 1 つのサーバーが適切に構成されている場合にのみ、Claude for Desktop に表示されます。

今回の場合は、単一の天気予報サーバーを次のように追加します。
...
`command` フィールドに `uv` 実行ファイルへのフルパスを入力する必要があるかもしれません。これは、MacOS/Linux では `which uv`、Windows では `where uv` を実行することで取得できます。

</Warning>

<Note>

サーバーの絶対パスを必ず指定してください。MacOS/Linuxでは「pwd」、Windowsコマンドプロンプトでは「cd」を実行することで取得できます。Windowsでは、JSONパスに二重のバックスラッシュ（「\\」）またはスラッシュ（「/」）を使用することを忘れないでください。

</Note>

これはClaude for Desktopに以下の情報を伝えます。

1. 「weather」という名前のMCPサーバーがあります。
2. 起動するには、`uv --directory /ABSOLUTE/PATH/TO/PARENT/FOLDER/weather run weather.py` を実行します。
...
天気予報サーバーの構築を始めましょう！[構築するサーバーの完全なコードは、こちらでご覧いただけます。](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-typescript)
...
1. Python の `logging` など、stderr またはファイルに書き込むログライブラリを使用します。
2. JavaScript の場合は特に注意が必要です。`console.log()` はデフォルトで stdout に書き込みます。
...
TypeScript をご利用の場合は、最新バージョンの Node がインストールされていることを確認してください。

### 環境の設定

まず、Node.js と npm をまだインストールしていない場合はインストールしましょう。これらは [nodejs.org](https://nodejs.org/) からダウンロードできます。
Node.js のインストール状況を確認します。
...
このチュートリアルでは、Node.js バージョン 16 以降が必要です。
...
`src/index.ts` の先頭に以下のコードを追加してください。
...
`npm run build` を実行してサーバーをビルドしてください。これはサーバーを接続する上で非常に重要なステップです。
